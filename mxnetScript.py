...
def train(batch_size, epochs,learning_rate, momentum, wd, log_interval, num_gpus, hosts, current_host, num_cpus, model_dir,
          train_data_dir, test_data_dir, **kwargs):
    ...
    train_data=_get_train_data(num_cpus, train_data_dir, batch_size,(3,224,224), num_parts=len(hosts), part_index=part_index,
                               resize=224)
    ...
    train = gluon.Trainer(net.collect_params(), 'sgd', otpimizer_params={'learning_rate':learning_rate, 'momentum': momentum,
                                                                         'wd':wd}, kvstore=kvstore)
    ...
    for epoch in range(epochs):
        ...
        for i,batch in enumerate(train_data):
            ...
            label= gluon.utils.split_and_load(batch.labels[0], ctx_list=ctx, batch_axis=0)
            outputs=[]
            Ls=[]
            ...
            if i % log_interval ==0 and i > 0:
                name, acc= metric.get()
                logging.info('Epoch [%d] Batch [%d]\tSpeed : %f samples/sec\r%s=%f' %
...
if __name__=='__main__':
    ...
    train(args.batch_size, args.epochs,args.learning_rate,args.momentum, args.wd, log_interval, num_gpus,  args.hosts,args.current_host, num_cpus,args.model_dir,args.train, args.test)
